    ...
    <div className="min-h-screen bg-background grid-pattern">
      ...
      <div className="glass-card border-b border-primary/20 p-3 sm:p-4 pt-16 sm:pt-20">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
          ...
          <div className="flex items-center space-x-2 w-full sm:w-auto justify-start sm:justify-end gap-2">
            <motion.div
            ...
            >
              <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></div>🧠 Agent Monitoring Active
            </motion.div>
            <Button
              onClick={onLogout}
              variant="outline"
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <Card className="glass-card bg-gradient-to-r from-primary/10 to-blue-500/10 text-foreground border border-primary/30 shadow-xl">
            <CardContent className="p-4 sm:p-6">
              <h2 className="text-responsive-lg sm:text-responsive-xl mb-4 font-semibold">
            ...
            <Card className="glass-card border border-primary/20 shadow-lg">
              ...
              <CardContent className="space-y-4">
                {Object.entries(vitals).map(([key, vital]) => (
                  <div
                    key={key}
                    className="flex items-center justify-between p-3 glass-card rounded-lg border border-border/50"
                  >
                    <div>
              ...
              <div className="space-y-4">
                {aiActions.map((action) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`p-2 rounded-full ${action.status === "completed" ? "bg-green-100" : "bg-blue-100"}`}
                      >
                        <IconComponent
                          className={`w-4 h-4 ${action.status === "completed" ? "text-green-600" : "text-blue-600"}`}
                        />
                      </div>
